...
The survey results can be downloaded from https://insights.stackoverflow.com/survey/2018. The survey result consist of multiple rows representing a single survey instance of a specific user and columns representing each question of the survey and answer of the user. The original StackOverflow user survey results consists of a CSV file with over 90.000 entries and over 40 columns. Obviously, there were lots of data we didn't need for our application use case, therefore we removed unused columns such as "Do you program as a hobby?". Also, for the sake of simplicity and efficiency we stripped the 90.000+ entries down to around 700, which we decided to do because for the assignment it makes no difference how many entries we have.
...
Obtaining this dataset was as simple as going to the website and clicking download. It is available for free and easy to find and download.
